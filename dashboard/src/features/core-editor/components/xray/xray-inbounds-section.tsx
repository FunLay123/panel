...
export function XrayInboundsSection({ headerAddPulse, headerAddEpoch }: XrayInboundsSectionProps) {
  ...
  const [initialDraftInbound, setInitialDraftInbound] = useState<Inbound | null>(null)
  ...
  useEffect(() => {
    ...
    if (dialogMode === 'add' && isTagAutoGenerated && watchedProtocol) {
      ...
      patchInbound({ tag })
      setInitialDraftInbound(prev => (prev ? ({ ...prev, tag } as Inbound) : prev))
    }
  ...
  const beginAddInbound = useCallback(() => {
    ...
    setInitialDraftInbound(created)
    setDialogMode('add')
  ...
  const finalizeDetailClose = () => {
    ...
    setInitialDraftInbound(null)
    setEditOriginalInbound(null)
  }

  const hasUnsavedDraftChanges = dialogMode === 'add' && draftInbound !== null && initialDraftInbound !== null && JSON.stringify(draftInbound) !== JSON.stringify(initialDraftInbound)
  const hasUnsavedEditChanges = dialogMode === 'edit' && inbound && inbound.protocol !== 'unmanaged' && editOriginalInbound && JSON.stringify(inbound) !== JSON.stringify(editOriginalInbound)
  ...
  const handleDetailOpenChange = (open: boolean) => {
    ...
    if (dialogMode === 'add' && draftInbound !== null && hasUnsavedDraftChanges) {
